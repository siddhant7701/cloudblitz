...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty(trim($_POST["slug"]))) {
        ...
        if ($stmt = $mysqli->prepare($sql)) {
            $stmt->bind_param("si", $param_slug, $param_id);
    ...
    if (empty($title_err) && empty($slug_err) && empty($content_err) && empty($category_id_err)) {
        ...
        $sql = "UPDATE blog_posts SET title = ?, slug = ?, content = ?, excerpt = ?, category_id = ?, image_path = ?, status = ?, updated_at = NOW() WHERE id = ?";
        
        if ($stmt = $mysqli->prepare($sql)) {
            $stmt->bind_param("sssssssi", $param_title, $param_slug, $param_content, $param_excerpt, $param_category_id, $param_image_path, $param_status, $param_id);
...
<head>
    ...
    <title>Edit Blog Post - CloudBlitz Admin</title>
                        ...
                        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" enctype="multipart/form-data" class="needs-validation">
                            ...
                            <div class="form-group">
                                ...
                                <?php if (!empty($image_path)): ?>
                                    <div class="mb-2">
                                        <img src="../<?php echo $image_path; ?>" alt="Current Image" id="image-preview" style="max-width: 200px; max-height: 200px;">
                                    </div>
                                <?php endif; ?>
